{{-- 🛡️ The Stylekart Admin Report Selector Modal --}}
<div id="reportModal" class="hidden fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-3xl shadow-2xl max-w-md w-full overflow-hidden border border-slate-100">
        ...
        <div class="p-6 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
            <h3 id="modalTitle" class="text-sm font-black text-slate-800 uppercase tracking-widest">Generate Global Report</h3>
            <button onclick="closeModal()" class="text-slate-400 hover:text-red-500 transition text-2xl">&times;</button>
        </div>

        {{-- Modal Body --}}
        {{-- 🎯 CRITICAL CHANGE: Route is now admin.reports.generate --}}
        <form id="reportForm" action="{{ route('admin.reports.generate') }}" method="GET" class="p-8">

            {{-- This hidden input is populated by prepareReport() --}}
            ...
            <div class="mt-6 pt-6 border-t border-slate-50 text-center">
                <p class="text-[9px] font-bold text-slate-300 uppercase tracking-[0.2em] mb-4">Quarterly Audit</p>
