...
<div class="bg-rose-50/20 min-h-screen py-12">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex items-center justify-between mb-8">
            ...
            <h1 class="text-xl font-black text-gray-900 uppercase tracking-tight">Edit Shipping Address</h1>
        </div>

        <div class="bg-white rounded-[40px] border border-rose-50 shadow-sm overflow-hidden">
            <form action="{{ route('customer.address.update', $address) }}" method="POST" class="p-8 md:p-12">
                @csrf
                @method('PATCH')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="address_line" rows="3"
                            ...
                            class="w-full bg-rose-50/30 border border-rose-50 rounded-3xl px-5 py-4 text-sm font-medium placeholder:text-gray-300 focus:outline-none focus:ring-2 focus:ring-rose-300 transition-all @error('address_line') border-rose-500 @enderror">{{ old('address_line', $address->address_line) }}</textarea>
                    ...
                    <div>
                        ...
                        <select name="address_type" class="w-full bg-rose-50/30 border border-rose-50 rounded-2xl px-5 py-4 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-rose-300 transition-all">
                            ...
                            <option value="home" {{ old('address_type', $address->address_type) === 'home' ? 'selected' : ''}}>🏠 Home (Delivery anytime)</option>
                            <option value="office" {{ old('address_type', $address->address_type) === 'office' ? 'selected' : ''}}>🏢 Office (10 AM - 6 PM)</option>
                ...
                <div class="mt-8 flex items-center justify-between p-4 bg-rose-50/20 rounded-2xl border border-rose-50">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="is_default" value="1" class="sr-only peer" {{ old('is_default', $address->is_default) ? 'checked' : '' }}>
